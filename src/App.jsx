...
// ═══════════════════════════════════════════════════════════════════════════════
// POSTURE CAMERA MODULE — Physiotherapy-Grade Live Pose Tracking
// ═══════════════════════════════════════════════════════════════════════════════

// ─── Smoothing filter for skeleton jitter reduction ───────────────────────────
function useSmoothLandmarks(alpha = 0.35) {
  const smoothed = useRef({});
  return (raw) => {
    if (!raw) return null;
    const out = raw.map((lm, i) => {
      const prev = smoothed.current[i];
      if (!prev) { smoothed.current[i] = { ...lm }; return { ...lm }; }
      const s = {
...
const KEY_JOINTS = [0,11,12,13,14,15,16,23,24,25,26,27,28,31,32];

// ─── SkeletonOverlay ──────────────────────────────────────────────────────────
function SkeletonOverlay({ canvasRef, landmarks, videoSize }) {
  useEffect(() => {
    ...
    const vis = (i) => landmarks[i] && landmarks[i].visibility > 0.4;

    // Draw connections
    POSE_CONNECTIONS.forEach(([a, b]) => {
      if (!vis(a) || !vis(b)) return;
      const pa = px(landmarks[a]), pb = px(landmarks[b]);
      const isArm = (a >= 11 && a <= 22) || (b >= 11 && b <= 22);
      const isLeg = (a >= 23) || (b >= 23);
      ctx.beginPath();
      ctx.moveTo(pa.x, pa.y);
      ctx.lineTo(pb.x, pb.y);
      ctx.strokeStyle = isArm ? "rgba(0,229,255,0.85)" : isLeg ? "rgba(0,201,122,0.85)" : "rgba(127,90,240,0.9)";
      ctx.lineWidth = 2.5;
    ...
    // Draw spine center line (11→23 midpoint path)
    if (vis(11) && vis(12) && vis(23) && vis(24)) {
      const shMid = { x: (landmarks[11].x + landmarks[12].x) / 2 * w, y: (landmarks[11].y + landmarks[12].y) / 2 * h };
      const hipMid = { x: (landmarks[23].x + landmarks[24].x) / 2 * w, y: (landmarks[23].y + landmarks[24].y) / 2 * h };
      ctx.beginPath();
    ...
    KEY_JOINTS.forEach(i => {
      ...
      ctx.fillStyle = i === 0 ? "#ffb300" : "rgba(0,229,255,0.95)";
...
// ─── TrackingWarnings ─────────────────────────────────────────────────────────
function TrackingWarnings({ landmarks }) {
  const warnings = [];
  if (!landmarks || landmarks.length === 0) return null;

  const vis = (i) => landmarks[i] && landmarks[i].visibility > 0.5;
  const avgVis = landmarks.slice(11, 33).reduce((s, lm) => s + (lm?.visibility || 0), 0) / 22;

  if (avgVis < 0.4) warnings.push({ icon: "⚠️", text: "Low tracking confidence — improve lighting", color: "#ff4d6d" });
  if (!vis(31) && !vis(32)) warnings.push({ icon: "👣", text: "Feet not visible — move camera back", color: "#ffb300" });
  else if (!vis(27) && !vis(28)) warnings.push({ icon: "📏", text: "Move backward — ankles out of frame", color: "#ffb300" });
  if (!vis(11) || !vis(12)) warnings.push({ icon: "🦴", text: "Shoulders not detected", color: "#ffb300" });
  if (!vis(0)) warnings.push({ icon: "👤", text: "Head not visible — check framing", color: "#ff4d6d" });

  const shoulderY = vis(11) && vis(12) ? (landmarks[11].y + landmarks[12].y) / 2 : null;
  if (shoulderY !== null && shoulderY > 0.3) warnings.push({ icon: "⬆️", text: "Too close — step back for full body", color: "#ff4d6d" });
  if (shoulderY !== null && shoulderY < 0.05) warnings.push({ icon: "⬇️", text: "Too far — step closer", color: "#ffb300" });

  if (warnings.length === 0) return null;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5, marginTop: 8 }}>
      {warnings.map((w, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", background: `${w.color}18`, border: `1px solid ${w.color}40`, borderRadius: 8, fontSize: "0.74rem", fontWeight: 600, color: w.color }}>
...
// ─── CameraControls ───────────────────────────────────────────────────────────
function CameraControls({ isActive, isLoading, onStart, onStop, onFlip, facingMode }) {
  const btn = (onClick, label, color, disabled = false) => (
    <button onClick={onClick} disabled={disabled} style={{ padding: "10px 20px", background: disabled ? "#1a2d45" : `linear-gradient(135deg,${color},${color}cc)`, border: "none", borderRadius: 10, color: disabled ? "#6b8399" : "#000", fontWeight: 800, fontSize: "0.78rem", cursor: disabled ? "not-allowed" : "pointer", flex: 1, minWidth: 100, transition: "all 0.2s" }}>
      {label}
    </button>
  );
  return (
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
      {!isActive
        ? btn(onStart, isLoading ? "⏳ Initialising…" : "📷 Start Camera", "#00e5ff", isLoading)
        : btn(onStop, "⏹ Stop Camera", "#ff4d6d")
      }
      {isActive && btn(onFlip, facingMode === "user" ? "🔄 Back Camera" : "🔄 Front Camera", "#7f5af0")}
    </div>
  );
}
...
function CameraPositionGuide() {
  const tips = [
    { icon: "📏", text: "Place camera ~2 metres away" },
    { icon: "🧍", text: "Keep full body in frame" },
    { icon: "🔦", text: "Good, even lighting required" },
    { icon: "📱", text: "Camera at hip height (for lateral), or front (for anterior)" },
    { icon: "👕", text: "Wear form-fitting clothing for best tracking" },
  ];
  return (
    <div style={{ background: "rgba(0,229,255,0.05)", border: "1px solid rgba(0,229,255,0.2)", borderRadius: 12, padding: 16, marginBottom: 14 }}>
      <div style={{ fontSize: "0.72rem", fontWeight: 800, color: "#00e5ff", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 10 }}>📐 Camera Setup Guide</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 8 }}>
        {tips.map((t, i) => (
          <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: "0.76rem", color: "#d4e0f0" }}>
            <span style={{ fontSize: "1rem", flexShrink: 0 }}>{t.icon}</span><span style={{ lineHeight: 1.4 }}>{t.text}</span>
          </div>
        ))}
      </div>
    </div>
...
// ─── TrackingStatus ───────────────────────────────────────────────────────────
function TrackingStatus({ isActive, isTracking, countdown, quality }) {
  const statusColor = !isActive ? "#6b8399" : isTracking ? "#00c97a" : "#ffb300";
  const statusLabel = !isActive ? "Camera Off" : countdown > 0 ? `Calibrating ${countdown}s…` : isTracking ? "Live Tracking" : "Detecting…";
  const qualityLabel = quality === null ? "" : quality > 0.75 ? "Excellent" : quality > 0.5 ? "Good" : quality > 0.3 ? "Fair" : "Poor";
  const qualityColor = quality === null ? "" : quality > 0.75 ? "#00c97a" : quality > 0.5 ? "#ffb300" : "#ff4d6d";
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
        <span style={{ width: 9, height: 9, borderRadius: "50%", background: statusColor, display: "inline-block", boxShadow: isTracking ? `0 0 8px ${statusColor}` : "none", animation: isTracking ? "pulse 1.4s infinite" : "none" }} />
        <span style={{ fontSize: "0.74rem", fontWeight: 700, color: statusColor }}>{statusLabel}</span>
      </div>
      {quality !== null && isActive && (
        <div style={{ fontSize: "0.68rem", padding: "2px 9px", borderRadius: 10, background: `${qualityColor}18`, color: qualityColor, fontWeight: 700, border: `1px solid ${qualityColor}30` }}>
          Signal: {qualityLabel}
        </div>
      )}
      {isTracking && (
        <div style={{ fontSize: "0.68rem", padding: "2px 9px", borderRadius: 10, background: "rgba(0,201,122,0.12)", color: "#00c97a", fontWeight: 700, border: "1px solid rgba(0,201,122,0.25)", display: "flex", alignItems: "center", gap: 5 }}>
...
// ─── PoseTracker (MediaPipe loader + runner) ──────────────────────────────────
function PoseTracker({ videoRef, isActive, onLandmarks }) {
...
// ─── CameraView ───────────────────────────────────────────────────────────────
function CameraView({ videoRef, canvasRef, isActive, facingMode }) {
  return (
    <div style={{ position: "relative", width: "100%", background: "#06090f", borderRadius: 12, overflow: "hidden", aspectRatio: "3/4", maxHeight: "60vh" }}>
      {!isActive && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12 }}>
          <div style={{ fontSize: "3rem" }}>📷</div>
          <div style={{ fontSize: "0.82rem", color: "#6b8399", textAlign: "center", padding: "0 20px" }}>Camera preview will appear here</div>
        </div>
      )}
      <video
        ref={videoRef}
        autoPlay playsInline muted
        style={{ width: "100%", height: "100%", objectFit: "cover", display: isActive ? "block" : "none", transform: facingMode === "user" ? "scaleX(-1)" : "none" }}
      />
      <canvas
        ref={canvasRef}
        style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", transform: facingMode === "user" ? "scaleX(-1)" : "none" }}
      />
      ...
      {isActive && (
        <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", opacity: 0.12 }} xmlns="http://www.w3.org/2000/svg">
          <line x1="33.3%" y1="0" x2="33.3%" y2="100%" stroke="#00e5ff" strokeWidth="1"/>
          <line x1="66.6%" y1="0" x2="66.6%" y2="100%" stroke="#00e5ff" strokeWidth="1"/>
          <line x1="0" y1="33.3%" x2="100%" y2="33.3%" stroke="#00e5ff" strokeWidth="1"/>
          <line x1="0" y1="66.6%" x2="100%" y2="66.6%" stroke="#00e5ff" strokeWidth="1"/>
        </svg>
      )}
    </div>
  );
...
function PostureCameraModule() {
  ...
  const smoothRef = useSmoothLandmarks(0.3);
  ...
  const quality = landmarks
    ? landmarks.slice(11, 33).reduce((s, lm) => s + (lm?.visibility || 0), 0) / 22
    : null;
  ...
  const startCamera = async (mode = facingMode) => {
    ...
    try {
      ...
      const tick = setInterval(() => {
        c--;
        setCountdown(c);
        if (c <= 0) { clearInterval(tick); setMpLoading(false); }
      }, 1000);
    } catch (err) {
      setIsLoading(false);
      if (err.name === "NotAllowedError") setPermError("Camera permission denied. Please allow camera access in your browser settings.");
      else if (err.name === "NotFoundError") setPermError("No camera found on this device.");
      else setPermError(`Camera error: ${err.message}`);
    }
  };
  ...
  const flipCamera = () => {
    ...
    setFacingMode(next);
    if (isActive) startCamera(next);
  };

  const handleLandmarks = useCallback((raw) => {
    const smoothed = raw ? smoothRef(raw) : null;
    setLandmarks(smoothed);
    setIsTracking(!!smoothed && smoothed.some(lm => lm?.visibility > 0.5));
  }, []);
  ...
  return (
    <div style={{ fontFamily: "'SF Pro Display','Helvetica Neue',system-ui,sans-serif" }}>
      ...
      {/* Module Header */}
      <div style={{ background: "linear-gradient(135deg,rgba(0,229,255,0.08),rgba(127,90,240,0.08))", border: "1px solid rgba(0,229,255,0.18)", borderRadius: 14, padding: "14px 18px", marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#00e5ff22,#7f5af022)", border: "1px solid rgba(0,229,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.3rem" }}>🎥</div>
          <div>
            <div style={{ fontWeight: 800, fontSize: "0.95rem", color: "#00e5ff" }}>Posture Analysis Camera</div>
            <div style={{ fontSize: "0.65rem", color: "#6b8399", letterSpacing: "0.5px" }}>Live Pose Tracking · MediaPipe BlazePose · Physiotherapy Grade</div>
          </div>
        </div>
        <TrackingStatus isActive={isActive} isTracking={isTracking} countdown={countdown} quality={quality} />
      </div>

      {/* Camera Setup Guide — shown before camera is active */}
      {!isActive && <CameraPositionGuide />}

      {/* Permission Error */}
      {permError && (
        <div style={{ background: "rgba(255,77,109,0.1)", border: "1px solid rgba(255,77,109,0.35)", borderRadius: 10, padding: "12px 14px", marginBottom: 12, fontSize: "0.78rem", color: "#ff4d6d", display: "flex", gap: 8 }}>
          <span>🚫</span><span>{permError}</span>
        </div>
      )}

      {/* MediaPipe loading notice */}
      {mpLoading && isActive && (
        <div style={{ background: "rgba(127,90,240,0.1)", border: "1px solid rgba(127,90,240,0.3)", borderRadius: 10, padding: "10px 14px", marginBottom: 10, fontSize: "0.76rem", color: "#7f5af0", display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ animation: "pulse 1s infinite", display: "inline-block" }}>⚙️</span>
          <span>Loading pose detection model… {countdown > 0 ? `Calibrating in ${countdown}s` : ""}</span>
        </div>
      )}
      ...
      {/* Skeleton overlay drawn onto canvas */}
      {isActive && <SkeletonOverlay canvasRef={canvasRef} landmarks={landmarks} videoSize={videoSize} />}

      {/* Pose tracker (invisible logic component) */}
      {isActive && !mpLoading && <PoseTracker videoRef={videoRef} isActive={isActive} onLandmarks={handleLandmarks} />}

      {/* Controls */}
      <CameraControls isActive={isActive} isLoading={isLoading} onStart={() => startCamera(facingMode)} onStop={stopCamera} onFlip={flipCamera} facingMode={facingMode} />

      {/* Tracking Warnings */}
      {isActive && <TrackingWarnings landmarks={landmarks} />}

      {/* Live joint visibility summary */}
      {isTracking && landmarks && (
        <div style={{ marginTop: 12, background: "#0d1117", border: "1px solid #1a2d45", borderRadius: 10, padding: "10px 14px" }}>
          <div style={{ fontSize: "0.62rem", fontWeight: 700, color: "#6b8399", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 8 }}>Detected Landmarks</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {Object.entries(JOINT_NAMES).map(([i, name]) => {
              const lm = landmarks[Number(i)];
              const v = lm?.visibility || 0;
              const col = v > 0.7 ? "#00c97a" : v > 0.4 ? "#ffb300" : "#ff4d6d";
              return (
                <div key={i} style={{ fontSize: "0.64rem", padding: "2px 8px", borderRadius: 8, background: `${col}15`, color: col, border: `1px solid ${col}30`, fontWeight: 600 }}>
      ...
      {/* Footer note */}
      <div style={{ marginTop: 12, fontSize: "0.64rem", color: "#6b8399", padding: "8px 12px", background: "#0d1117", borderRadius: 8, lineHeight: 1.5, border: "1px solid #1a2d45" }}>
        <strong style={{ color: "#d4e0f0" }}>Note:</strong> This module uses your device camera locally. No video is uploaded or stored. Pose tracking runs entirely in your browser using MediaPipe BlazePose. Advanced posture scoring and analysis will be added in future modules.
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return(
    <div style={{background:C.bg,color:C.text,minHeight:"100vh",fontFamily:"'SF Pro Display','Helvetica Neue',system-ui,sans-serif",display:"flex",flexDirection:"column"}}>

      {/* Info Modal */}
      {infoModal&&(
        <div onClick={()=>setInfoModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div onClick={e=>e.stopPropagation()} style={{background:C.surface,border:`1px solid ${C.accent}40`,borderRadius:14,padding:24,maxWidth:500,width:"100%",maxHeight:"82vh",overflowY:"auto"}}>
            ...
            <button onClick={()=>setInfoModal(null)} style={{padding:"8px 20px",background:C.a2,border:"none",borderRadius:8,color:"#fff",fontWeight:700,cursor:"pointer",width:"100%"}}>Close</button>
          </div>
        </div>
      )}

      {/* Header */}
      <div style={{background:`linear-gradient(135deg,${C.surface},${C.s2})`,borderBottom:`1px solid ${C.border}`,padding:"0 20px",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:1400,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:54}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:32,height:32,background:`linear-gradient(135deg,${C.accent},${C.a2})`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center"}}>⚕</div>
            <div>
              <div style={{fontWeight:800,fontSize:"0.95rem",background:`linear-gradient(90deg,${C.accent},${C.a2})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>PhysioMaster Pro v4</div>
              <div style={{fontSize:"0.58rem",color:C.muted,letterSpacing:"0.5px"}}>NKT · CYRIAX · FMS · POSTURE · KINETIC CHAIN · FASCIA · MUSCLE ACTIVATION</div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:"0.7rem",color:C.muted}}><span style={{color:C.accent,fontWeight:700}}>{completedCount}</span> fields</span>
            <button onClick={runDx} style={{padding:"7px 16px",background:`linear-gradient(135deg,${C.accent},${C.a2})`,border:"none",borderRadius:8,color:"#000",fontWeight:800,fontSize:"0.78rem",cursor:"pointer"}}>▶ Diagnose</button>
          </div>
        </div>
      </div>
      ...
      <div style={{display:"flex",flex:1,maxWidth:1400,margin:"0 auto",width:"100%"}}>
        ...
        {/* Sidebar */}
        <div style={{width:195,minWidth:195,borderRight:`1px solid ${C.border}`,padding:"10px 0",background:C.surface,position:"sticky",top:54,height:"calc(100vh - 54px)",overflowY:"auto"}}>
          {sections.map(([key,sec])=>{
            ...
            return(
              <div key={key} onClick={()=>setActive(key)} style={{padding:"8px 12px",cursor:"pointer",margin:"1px 4px",borderRadius:8,background:isAct?"rgba(0,229,255,0.08)":"transparent",border:`1px solid ${isAct?"rgba(0,229,255,0.25)":"transparent"}`}}>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:"0.88rem"}}>{sec.icon}</span>
                  <div style={{flex:1}}>
                    <div style={{fontSize:"0.75rem",fontWeight:isAct?700:500,color:isAct?C.accent:C.text}}>{sec.label}</div>
                  ...
                  <span style={{fontSize:"0.56rem",color:C.muted}}>{pct}%</span>
                </div>
              </div>
            );
          })}
          <div style={{margin:"10px 8px"}}>
            <button onClick={runDx} style={{width:"100%",padding:"9px",background:`linear-gradient(135deg,${C.accent},${C.a2})`,border:"none",borderRadius:8,color:"#000",fontWeight:800,fontSize:"0.76rem",cursor:"pointer"}}>▶ DIAGNOSE</button>
          </div>
        </div>
