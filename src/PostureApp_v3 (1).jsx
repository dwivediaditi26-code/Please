...
function CameraView({ videoRef, canvasRef, isActive, facingMode, children, onTapFocus, zoom }) {
  ...
  return (
    <div
      ...
      onClick={handleTap}
      style={{ position:"relative", width:"100%", background:"#06090f", borderRadius:14, overflow:"hidden", aspectRatio:"3/4", maxHeight:"65vh", cursor: isActive ? "crosshair" : "default", touchAction:"manipulation" }}
    >
      {!isActive && (
        <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:10 }}>
          <div style={{ fontSize:"2.8rem" }}>📷</div>
          <div style={{ fontSize:"0.8rem", color:"#6b8399", textAlign:"center", padding:"0 20px", lineHeight:1.5 }}>Tap Start Camera to begin<br/>physiotherapy assessment</div>
        </div>
      )}
      <video
        ref={videoRef}
...
function CameraControls({ isActive, isLoading, onStart, onStop, onFlip, onRecalibrate, facingMode, canRecalibrate, zoom, onZoom, countdownSecs, onCountdownChange, burstMode, onBurstToggle, activeView, onViewChange }) {
  const views = ["anterior","posterior","left","right"];
  ...
  return (
    <div style={{ marginTop:10 }}>
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:8 }}>
        {!isActive
          ? <Btn onClick={onStart} label={isLoading?"⏳ Loading…":"📷 Start Camera"} bg="#00e5ff" disabled={isLoading}/>
          : <Btn onClick={onStop} label="⏹ Stop" bg="#ff4d6d"/>
        }
        {isActive && <Btn onClick={onFlip} label={facingMode==="user"?"🔄 Back":"🔄 Front"} bg="#7f5af0"/>}
        {canRecalibrate && <Btn onClick={onRecalibrate} label="⟳ Recal." bg="#ffb300"/>}
      </div>

      {/* Multi-view workflow */}
      {isActive && (
        <div style={{ marginBottom:8 }}>
          <div style={{ fontSize:"0.58rem", fontWeight:700, color:"#6b8399", textTransform:"uppercase", letterSpacing:"1px", marginBottom:5 }}>📐 Guided View Capture</div>
          <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
            {views.map(v => (
              <button key={v} onClick={() => onViewChange && onViewChange(v)} style={{
                padding:"5px 10px", borderRadius:8, fontSize:"0.65rem", fontWeight:700, cursor:"pointer", border:"none",
                background: activeView===v ? "linear-gradient(135deg,#00e5ff,#7f5af0)" : "#1a2d45",
                color: activeView===v ? "#000" : "#6b8399", flex:"1 0 auto"
      ...
      {isActive && (
        <div style={{ display:"flex", gap:7, flexWrap:"wrap", alignItems:"center" }}>
          {/* Zoom */}
          <div style={{ display:"flex", alignItems:"center", gap:5, background:"#0d1117", border:"1px solid #1a2d45", borderRadius:9, padding:"5px 10px", flex:"1 1 auto" }}>
            <span style={{ fontSize:"0.65rem", color:"#6b8399", whiteSpace:"nowrap" }}>🔍 Zoom</span>
          ...
          {/* Countdown timer */}
          <div style={{ display:"flex", alignItems:"center", gap:5, background:"#0d1117", border:"1px solid #1a2d45", borderRadius:9, padding:"5px 10px" }}>
            <span style={{ fontSize:"0.65rem", color:"#6b8399" }}>⏱</span>
            {[3,5,10].map(s => (
...
function PostureCameraModule() {
  ...
  const startCamera = async (mode = facingMode) => {
    ...
    try {
      ...
      let stream;
      try {
        stream = await navigator.mediaDevices.getUserMedia({ video:{ facingMode:mode, width:{ideal:1280}, height:{ideal:1920}, frameRate:{ideal:30,max:60} }, audio:false });
      } catch {
        stream = await navigator.mediaDevices.getUserMedia({ video:{ facingMode:mode, width:{ideal:720}, height:{ideal:960} }, audio:false });
      }
  ...
  const triggerCountdownCapture = useCallback(() => {
    ...
    const t = setInterval(() => {
      c--;
      setCaptureCountdown(c);
      if (c <= 0) { clearInterval(t); setCaptureCountdown(null); }
    }, 1000);
  }, [captureCountdown, countdownSecs]);
    ...
    <div>
      ...
      <div style={{ background:"linear-gradient(135deg,rgba(0,229,255,0.07),rgba(127,90,240,0.07))", border:"1px solid rgba(0,229,255,0.16)", borderRadius:14, padding:"13px 16px", marginBottom:12 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:7 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:"0.63rem", color:"#6b8399" }}>MediaPipe BlazePose · HD Capture · Physiotherapy Grade</div>
          </div>
      ...
      <div style={{ position:"relative" }}>
        <CameraView videoRef={videoRef} canvasRef={canvasRef} isActive={isActive} facingMode={facingMode} onTapFocus={handleTapFocus} zoom={zoom}>
          ...
          {isActive && trackState===TRACKING_STATES.STABLE && quality.ready && (
            <button onClick={triggerCountdownCapture} style={{ position:"absolute", bottom:16, left:"50%", transform:"translateX(-50%)", width:60, height:60, borderRadius:"50%", background:"linear-gradient(135deg,#00e5ff,#7f5af0)", border:"4px solid rgba(255,255,255,0.2)", cursor:"pointer", fontSize:"1.5rem", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 4px 20px rgba(0,229,255,0.4)", zIndex:20, transition:"transform 0.15s" }}>
              📸
            </button>
          )}
        </CameraView>
      ...
      <CameraControls
        ...
        onStart={()=>startCamera(facingMode)} onStop={stopCamera}
      ...
      {trackState===TRACKING_STATES.STABLE && landmarks && quality.ready && (
        <PostureAnalysisEngine landmarks={landmarks} canvasRef={canvasRef} videoSize={videoSize} videoRef={videoRef} isActive={isActive}/>
      )}
...
// ─── FramingStatus — detects which body parts are visible ────────────────────
function getFramingStatus(landmarks) {
  if(!landmarks) return { missing:["No pose detected"], score:0, ready:false };
  const vis=(i,t=0.45)=>(landmarks[i]?.visibility||0)>t;
  const g=i=>landmarks[i];
  const missing=[];

  // Critical landmarks for full-body posture
  if(!vis(0,0.4)) missing.push("Head");
  if(!vis(11)&&!vis(12)) missing.push("Shoulders");
  else if(!vis(11)) missing.push("Left shoulder");
  else if(!vis(12)) missing.push("Right shoulder");
  if(!vis(23)&&!vis(24)) missing.push("Hips");
  if(!vis(25)&&!vis(26)) missing.push("Knees");
  if(!vis(27)&&!vis(28)) missing.push("Ankles");
  if(!vis(31)&&!vis(32)) missing.push("Feet");

  // Position hints
  const hints=[];
  if(!vis(31)&&!vis(32)&&!vis(27)&&!vis(28)) hints.push({icon:"📷↑",text:"Step back — feet not in frame",col:"#ffb300",priority:3});
  if(!vis(0,0.35)) hints.push({icon:"📷↓",text:"Step back — head cropped",col:"#ffb300",priority:3});
  if(vis(11)&&vis(12)){
    const shMid=(g(11).x+g(12).x)/2;
    if(shMid>0.72) hints.push({icon:"←",text:"Move left — body off-center",col:"#00e5ff",priority:2});
    else if(shMid<0.28) hints.push({icon:"→",text:"Move right — body off-center",col:"#00e5ff",priority:2});
  }
  if(vis(0)&&vis(31)&&vis(32)){
    const bodyH=Math.abs((g(0).y)-(Math.max(g(31)?.y||0,g(32)?.y||0)));
    if(bodyH<0.5) hints.push({icon:"↔",text:"Step closer — too far from camera",col:"#ffb300",priority:1});
    else if(bodyH>0.95) hints.push({icon:"↔",text:"Step back — too close to camera",col:"#ffb300",priority:2});
  }

  const score = Math.max(0, 100 - missing.length * 15 - hints.filter(h=>h.priority>=2).length*10);
  const ready = missing.length===0 && score>=80;
  return { missing, hints, score, ready };
}

function SmartRepositionGuide({ landmarks, quality }) {
  const status = getFramingStatus(landmarks);
  if(!landmarks) return(
    <div style={{padding:"8px 12px",background:"rgba(255,179,0,0.07)",border:"1px solid rgba(255,179,0,0.2)",borderRadius:9,marginBottom:10,fontSize:"0.72rem",color:"#ffb300",fontWeight:600}}>
      📷 Camera not active — start camera to detect body position
    </div>
  );
  if(status.ready) return(
    <div style={{padding:"7px 12px",background:"rgba(0,201,122,0.07)",border:"1px solid rgba(0,201,122,0.2)",borderRadius:9,marginBottom:10,fontSize:"0.72rem",color:"#00c97a",fontWeight:700,display:"flex",alignItems:"center",gap:7}}>
      ✓ Full body in frame — all landmarks detected · Ready for capture
      <span style={{marginLeft:"auto",fontSize:"0.62rem",padding:"2px 7px",borderRadius:6,background:"rgba(0,201,122,0.12)",border:"1px solid rgba(0,201,122,0.3)"}}>{status.score}%</span>
    </div>
  );
  return(
    <div style={{marginBottom:10,display:"flex",flexDirection:"column",gap:5}}>
      ...
      {status.missing.length>0&&(
        <div style={{padding:"7px 11px",background:"rgba(255,77,109,0.07)",border:"1px solid rgba(255,77,109,0.25)",borderRadius:9}}>
          <div style={{fontSize:"0.62rem",fontWeight:700,color:"#ff4d6d",marginBottom:4}}>⚠ Missing from frame:</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
            {status.missing.map((m,i)=>(
              <span key={i} style={{fontSize:"0.68rem",padding:"2px 8px",borderRadius:7,background:"rgba(255,77,109,0.12)",color:"#ff4d6d",border:"1px solid rgba(255,77,109,0.25)",fontWeight:600}}>{m}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {status.hints.map((h,i)=>(
        <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 11px",background:`${h.col}10`,border:`1px solid ${h.col}28`,borderRadius:8,fontSize:"0.72rem",fontWeight:700,color:h.col}}>
          <span style={{fontSize:"1rem"}}>{h.icon}</span>{h.text}
        </div>
      ))}
      <div style={{padding:"5px 10px",background:"#0d1117",border:"1px solid #1a2d45",borderRadius:7,fontSize:"0.62rem",color:"#6b8399",display:"flex",alignItems:"center",gap:6}}>
        <div style={{flex:1,height:4,background:"#1a2d45",borderRadius:2,overflow:"hidden"}}>
          <div style={{height:"100%",width:`${status.score}%`,background:status.score>70?"#00c97a":status.score>40?"#ffb300":"#ff4d6d",borderRadius:2,transition:"width 0.4s ease"}}/>
        </div>
        <span style={{fontWeight:700,color:status.score>70?"#00c97a":status.score>40?"#ffb300":"#ff4d6d"}}>{status.score}%</span>
      </div>
    </div>
  );
}

// ─── SmartCaptureSystem — auto + manual capture with cooldown + stability ─────
function SmartCaptureSystem({ onCapture, landmarks, isActive, mode, onModeChange }) {
  const [stable, setStable] = useState(0);     // frames stable
  const [cooldown, setCooldown] = useState(0); // seconds until next auto
  ...
  // Stability tracking
  useEffect(()=>{
    if(!landmarks||!isActive){ setStable(0); return; }
    if(!prevLm.current){ prevLm.current=landmarks; return; }
    const drift = [0,11,12,23,24,25,26,27,28].reduce((sum,i)=>{
      const a=landmarks[i], b=prevLm.current[i];
      if(!a||!b) return sum;
      return sum + Math.abs(a.x-b.x)*100 + Math.abs(a.y-b.y)*100;
    },0)/9;
    prevLm.current=landmarks;
    setStable(s => drift < 1.2 ? Math.min(s+1,STABLE_NEEDED+10) : Math.max(0,s-3));
  },[landmarks,isActive]);

  // Auto-capture trigger
  const framing = getFramingStatus(landmarks);
  const isStable = stable >= STABLE_NEEDED;
  const canAutoCapture = mode==="auto" && framing.ready && isStable && cooldown===0 && isActive;
  ...
  useEffect(()=>{
    ...
    const t = setTimeout(()=>{
      ...
      let c=COOLDOWN_SECS;
      cooldownTimer.current=setInterval(()=>{ c--; setCooldown(c); if(c<=0){ clearInterval(cooldownTimer.current); }},1000);
    },600);
    return()=>{ clearTimeout(t); setAutoArmed(false); };
  },[canAutoCapture]);

  useEffect(()=>()=>{ if(cooldownTimer.current) clearInterval(cooldownTimer.current); },[]);

  const stabPct = Math.min(100, Math.round((stable/STABLE_NEEDED)*100));

  return(
    <div style={{background:"#0d1117",border:"1px solid #1a2d45",borderRadius:12,padding:"11px 13px",marginBottom:10}}>
      {/* Mode toggle */}
      <div style={{display:"flex",gap:6,marginBottom:10,background:"#06090f",borderRadius:9,padding:3,border:"1px solid #1a2d45"}}>
        {[["auto","🤖 Auto"],["manual","📸 Manual"]].map(([m,l])=>(
          <button key={m} onClick={()=>onModeChange(m)} style={{flex:1,padding:"7px",background:mode===m?"linear-gradient(135deg,rgba(0,229,255,0.2),rgba(127,90,240,0.2))":"transparent",border:mode===m?"1px solid rgba(0,229,255,0.3)":"1px solid transparent",borderRadius:7,color:mode===m?"#00e5ff":"#6b8399",fontWeight:mode===m?800:600,fontSize:"0.72rem",cursor:"pointer",transition:"all 0.15s"}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {mode==="auto"&&(
        <div>
          <div style={{marginBottom:7}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
              <span style={{fontSize:"0.62rem",color:"#6b8399",fontWeight:600}}>Stability</span>
              <span style={{fontSize:"0.62rem",fontWeight:700,color:isStable?"#00c97a":"#ffb300"}}>{stabPct}%</span>
            </div>
            <div style={{height:5,background:"#1a2d45",borderRadius:3,overflow:"hidden"}}>
              <div style={{height:"100%",width:`${stabPct}%`,background:isStable?"#00c97a":"linear-gradient(90deg,#ffb300,#00e5ff)",borderRadius:3,transition:"width 0.2s ease"}}/>
            </div>
          </div>
          {cooldown>0?(
            <div style={{padding:"8px 12px",background:"rgba(127,90,240,0.08)",border:"1px solid rgba(127,90,240,0.2)",borderRadius:8,fontSize:"0.72rem",color:"#7f5af0",fontWeight:700,textAlign:"center"}}>
              ⏱ Cooldown: {cooldown}s before next auto-capture
            </div>
          ):autoArmed?(
            <div style={{padding:"8px 12px",background:"rgba(0,229,255,0.1)",border:"1px solid rgba(0,229,255,0.3)",borderRadius:8,fontSize:"0.75rem",color:"#00e5ff",fontWeight:800,textAlign:"center",animation:"pcPulse 0.6s infinite"}}>
              📸 Capturing…
            </div>
          ):!isActive?(
            <div style={{padding:"8px 12px",background:"rgba(255,179,0,0.07)",border:"1px solid rgba(255,179,0,0.2)",borderRadius:8,fontSize:"0.72rem",color:"#ffb300",textAlign:"center",fontWeight:600}}>
              Start camera to enable auto-capture
            </div>
          ):!framing.ready?(
            <div style={{padding:"8px 12px",background:"rgba(255,179,0,0.07)",border:"1px solid rgba(255,179,0,0.2)",borderRadius:8,fontSize:"0.72rem",color:"#ffb300",textAlign:"center",fontWeight:600}}>
            ...
            <div style={{padding:"8px 12px",background:"rgba(0,201,122,0.07)",border:"1px solid rgba(0,201,122,0.2)",borderRadius:8,fontSize:"0.72rem",color:"#00c97a",textAlign:"center",fontWeight:700}}>
              {isStable?"✓ Stable — auto-capture ready":"○ Hold still for stable capture…"}
            </div>
          )}
        </div>
      )}
      ...
      <button
        onClick={()=>{ if(cooldown===0){ onCapture("manual"); setCooldown(COOLDOWN_SECS); let c=COOLDOWN_SECS; cooldownTimer.current=setInterval(()=>{ c--; setCooldown(c); if(c<=0) clearInterval(cooldownTimer.current); },1000); }}}
        disabled={!isActive||cooldown>0}
        style={{width:"100%",marginTop:mode==="auto"?8:0,padding:"12px",background:(!isActive||cooldown>0)?"#1a2d45":"linear-gradient(135deg,#00e5ff,#7f5af0)",border:"none",borderRadius:10,color:(!isActive||cooldown>0)?"#6b8399":"#000",fontWeight:900,fontSize:"0.82rem",cursor:(!isActive||cooldown>0)?"not-allowed":"pointer",transition:"all 0.2s"}}
      >
        {cooldown>0?`⏱ Cooldown ${cooldown}s`:!isActive?"📷 Start camera first":"📸 Capture Now"}
      </button>
    </div>
  );
}
...
// ─── AI Posture Analysis Engine (rule-based, no paid API) ────────────────────
function runPostureCaptureAnalysis(measurements, view, landmarks) {
  if(!measurements||!landmarks) return null;
  const findings=[], muscles=[], corrections=[];
  const lm=landmarks;
  const vis=(i,t=0.4)=>(lm[i]?.visibility||0)>t;

  // ── Anterior view ──
  if(view==="anterior"||view==="posterior"){
    const shDiff = measurements.shoulderSymmetry;
    const hipDiff = measurements.hipSymmetry;
    if(shDiff!=null&&Math.abs(shDiff)>2){ findings.push({region:"Shoulders",deviation:`${shDiff>0?"Right":"Left"} shoulder elevated ${Math.abs(shDiff).toFixed(1)}°`,severity:Math.abs(shDiff)>5?"high":"moderate",kinetic:"Upper trapezius/levator scapulae overactivity on elevated side"}); muscles.push(shDiff>0?"Right upper trapezius tight":"Left upper trapezius tight"); corrections.push("Unilateral neck/shoulder stretching · Thoracic mobility work"); }
    if(hipDiff!=null&&Math.abs(hipDiff)>2){ findings.push({region:"Pelvis",deviation:`${hipDiff>0?"Right":"Left"} hip elevated — lateral pelvic tilt`,severity:Math.abs(hipDiff)>5?"high":"moderate",kinetic:"Contralateral hip abductor weakness / ipsilateral QL shortening"}); muscles.push("Quadratus lumborum imbalance"); corrections.push("Hip abductor strengthening · QL stretching · Lateral core stability"); }
    if(vis(11)&&vis(12)&&vis(25)&&vis(26)){ const kL=lm[25].x, kR=lm[26].x, aL=lm[27]?.x||kL, aR=lm[28]?.x||kR; const kvL=(kL-aL)*100, kvR=(aR-kR)*100; if(kvL>6){ findings.push({region:"Left Knee",deviation:"Knee valgus (medial collapse)",severity:"high",kinetic:"Hip abductor/external rotator weakness → tibial stress risk"}); muscles.push("Weak left glute med · Tight left TFL/IT band"); corrections.push("Glute med activation · VMO strengthening · Knee tracking drills"); } if(kvR>6){ findings.push({region:"Right Knee",deviation:"Knee valgus (medial collapse)",severity:"high",kinetic:"Hip abductor/external rotator weakness → tibial stress risk"}); muscles.push("Weak right glute med · Tight right TFL/IT band"); corrections.push("Glute med activation · VMO strengthening · Knee tracking drills"); } }
  }

  // ── Lateral view ──
  if(view==="lateral"){
    const fwdHead = measurements.forwardHeadAngle;
    const kyphosis = measurements.thoracicKyphosis;
    const lordosis = measurements.lumbarLordosis;
    const pelvicTilt = measurements.anteriorPelvicTilt;
    if(fwdHead!=null&&fwdHead>10){ findings.push({region:"Cervical Spine",deviation:`Forward head posture +${fwdHead.toFixed(1)}°`,severity:fwdHead>20?"high":"moderate",kinetic:"Deep cervical flexor inhibition · Upper cross syndrome pattern"}); muscles.push("Tight SCM/upper trap · Weak deep neck flexors"); corrections.push("Chin tucks · Deep neck flexor strengthening · Chest stretching"); }
    if(kyphosis!=null&&kyphosis>40){ findings.push({region:"Thoracic Spine",deviation:`Increased thoracic kyphosis ${kyphosis.toFixed(1)}°`,severity:kyphosis>55?"high":"moderate",kinetic:"Thoracic extensors inhibited · Pectoral shortening"}); muscles.push("Weak thoracic extensors · Tight pectorals/anterior chest"); corrections.push("Thoracic extension mobilisation · Row exercises · Foam roller thoracic extension"); }
    if(lordosis!=null&&lordosis>45){ findings.push({region:"Lumbar Spine",deviation:`Increased lumbar lordosis ${lordosis.toFixed(1)}°`,severity:lordosis>60?"high":"moderate",kinetic:"Anterior pelvic tilt — hip flexor dominance → lumbar facet loading"}); muscles.push("Tight hip flexors/erector spinae · Weak glutes/core"); corrections.push("Hip flexor stretching · Posterior pelvic tilt exercises · Dead bug core series"); }
    if(pelvicTilt!=null&&pelvicTilt>8){ findings.push({region:"Pelvis",deviation:`Anterior pelvic tilt ${pelvicTilt.toFixed(1)}°`,severity:pelvicTilt>15?"high":"moderate",kinetic:"Lower cross syndrome — hip flexor dominance, glute/core inhibition"}); muscles.push("Tight iliopsoas/rectus femoris · Weak glutes/hamstrings"); corrections.push("Glute bridges · Hamstring loading · Hip flexor long-duration stretching"); }
  }

  // ── Symmetry-based ──
  const symScore = measurements.overallSymmetry;
  if(symScore!=null&&symScore<75){ findings.push({region:"Overall Alignment",deviation:`Asymmetric posture — symmetry score ${symScore.toFixed(0)}%`,severity:symScore<60?"high":"moderate",kinetic:"Compensatory load distribution affecting kinetic chain efficiency"}); corrections.push("Full movement screen · Bilateral corrective exercises"); }

  // Score
  const highCount = findings.filter(f=>f.severity==="high").length;
  const modCount  = findings.filter(f=>f.severity==="moderate").length;
  const postureScore = Math.max(0, 100 - highCount*20 - modCount*10);

  return { findings, muscles:[...new Set(muscles)], corrections:[...new Set(corrections)], postureScore, view };
}

// ─── PostureCaptureCard — single capture with embedded analysis ───────────────
function PostureCaptureCard({ capture, index, onRetake, onDelete }) {
  const [showAnalysis, setShowAnalysis] = useState(true);
  const { img, time, view, captureMode, analysis, postureScore } = capture;
  const scoreCol = postureScore>=80?"#00c97a":postureScore>=60?"#ffb300":"#ff4d6d";

  return(
    <div style={{background:"#0d1117",border:"1px solid #1a2d45",borderRadius:13,overflow:"hidden",marginBottom:12}}>
      {/* Image + meta */}
      <div style={{position:"relative"}}>
        <img src={img} alt={`capture-${index}`} style={{width:"100%",display:"block",maxHeight:"55vh",objectFit:"contain",background:"#06090f"}}/>
        {/* Overlays */}
        <div style={{position:"absolute",top:8,left:8,display:"flex",gap:5,flexWrap:"wrap"}}>
          <span style={{background:"rgba(6,9,15,0.85)",borderRadius:6,padding:"3px 8px",fontSize:"0.64rem",color:"#7f5af0",fontWeight:700,textTransform:"capitalize",border:"1px solid rgba(127,90,240,0.3)"}}>{view}</span>
          <span style={{background:captureMode==="auto"?"rgba(0,229,255,0.85)":"rgba(0,201,122,0.85)",borderRadius:6,padding:"3px 8px",fontSize:"0.6rem",fontWeight:700,color:"#000"}}>{captureMode==="auto"?"🤖 Auto":"📸 Manual"}</span>
        </div>
        <div style={{position:"absolute",top:8,right:8,width:38,height:38,borderRadius:"50%",background:"rgba(6,9,15,0.9)",border:`2px solid ${scoreCol}`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:"0.85rem",color:scoreCol}}>{postureScore}</div>
        <div style={{position:"absolute",bottom:8,left:8,background:"rgba(6,9,15,0.8)",borderRadius:6,padding:"3px 8px",fontSize:"0.6rem",color:"#6b8399"}}>🕐 {time}</div>
      </div>

      {/* Action bar */}
      <div style={{display:"flex",gap:6,padding:"8px 10px",borderBottom:"1px solid #1a2d45"}}>
        <button onClick={()=>setShowAnalysis(a=>!a)} style={{flex:2,padding:"7px",background:showAnalysis?"rgba(0,229,255,0.1)":"transparent",border:`1px solid ${showAnalysis?"rgba(0,229,255,0.3)":"#1a2d45"}`,borderRadius:8,color:showAnalysis?"#00e5ff":"#6b8399",fontWeight:700,fontSize:"0.7rem",cursor:"pointer"}}>
          {showAnalysis?"▲ Hide Analysis":"▼ Show Analysis"}
        </button>
        <button onClick={onRetake} style={{flex:1,padding:"7px",background:"transparent",border:"1px solid #1a2d45",borderRadius:8,color:"#6b8399",fontWeight:700,fontSize:"0.7rem",cursor:"pointer"}}>🔄 Retake</button>
        <a href={img} download={`posture-${view}-${Date.now()}.jpg`} style={{flex:1,padding:"7px",background:"rgba(0,201,122,0.1)",border:"1px solid rgba(0,201,122,0.3)",borderRadius:8,color:"#00c97a",fontWeight:700,fontSize:"0.7rem",cursor:"pointer",textDecoration:"none",textAlign:"center"}}>⬇ Save</a>
        <button onClick={onDelete} style={{padding:"7px 10px",background:"transparent",border:"1px solid rgba(255,77,109,0.25)",borderRadius:8,color:"#ff4d6d",fontSize:"0.7rem",cursor:"pointer"}}>✕</button>
      </div>
      ...
      {showAnalysis&&analysis&&(
        <div style={{padding:"11px 13px"}}>
          {/* Posture score */}
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10,padding:"8px 11px",background:`${scoreCol}0e`,border:`1px solid ${scoreCol}28`,borderRadius:9}}>
            <div style={{width:42,height:42,borderRadius:"50%",border:`2px solid ${scoreCol}`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:"1rem",color:scoreCol,flexShrink:0}}>{postureScore}</div>
            <div>
              <div style={{fontWeight:700,fontSize:"0.78rem",color:scoreCol}}>{postureScore>=80?"Good alignment":postureScore>=60?"Moderate deviations":"Significant deviations"}</div>
              <div style={{fontSize:"0.62rem",color:"#6b8399"}}>{analysis.findings.length} finding{analysis.findings.length!==1?"s":""} · {view} view</div>
            </div>
          </div>
          ...
          {analysis.findings.length>0&&(
            <div style={{marginBottom:10}}>
              <div style={{fontSize:"0.6rem",fontWeight:700,color:"#6b8399",textTransform:"uppercase",letterSpacing:"1px",marginBottom:6}}>🔍 Postural Findings</div>
              {analysis.findings.map((f,i)=>{
                const col=f.severity==="high"?"#ff4d6d":"#ffb300";
                return(
                  <div key={i} style={{background:`${col}09`,border:`1px solid ${col}28`,borderRadius:9,padding:"9px 11px",marginBottom:6}}>
                    <div style={{display:"flex",gap:7,alignItems:"flex-start",marginBottom:4}}>
                      <span style={{fontSize:"0.6rem",padding:"2px 6px",borderRadius:5,background:`${col}20`,color:col,fontWeight:700,flexShrink:0,textTransform:"capitalize"}}>{f.region}</span>
                      <span style={{fontSize:"0.72rem",color:"#d4e0f0",fontWeight:600,lineHeight:1.4}}>{f.deviation}</span>
                    </div>
                    <div style={{fontSize:"0.68rem",color:"#6b8399",lineHeight:1.5,paddingLeft:2}}>⛓ {f.kinetic}</div>
                  </div>
                );
              })}
            </div>
          ...
          {analysis.muscles.length>0&&(
            <div style={{marginBottom:10}}>
              <div style={{fontSize:"0.6rem",fontWeight:700,color:"#6b8399",textTransform:"uppercase",letterSpacing:"1px",marginBottom:6}}>💪 Muscle Imbalance Map</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {analysis.muscles.map((m,i)=>(
                  <span key={i} style={{fontSize:"0.68rem",padding:"3px 9px",borderRadius:7,background:"rgba(255,179,0,0.1)",color:"#ffb300",border:"1px solid rgba(255,179,0,0.25)",fontWeight:600}}>{m}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {analysis.corrections.length>0&&(
            <div>
              <div style={{fontSize:"0.6rem",fontWeight:700,color:"#6b8399",textTransform:"uppercase",letterSpacing:"1px",marginBottom:6}}>🏋️ Corrective Exercises</div>
              {analysis.corrections.map((c,i)=>(
                <div key={i} style={{display:"flex",gap:7,padding:"5px 0",borderBottom:"1px solid #1a2d45",fontSize:"0.72rem",color:"#d4e0f0",lineHeight:1.5}}>
                  <span style={{color:"#00c97a",fontWeight:700,flexShrink:0}}>{i+1}.</span>{c}
                </div>
              ))}
            </div>
          )}
          ...
          {analysis.findings.length===0&&(
            <div style={{padding:"10px",background:"rgba(0,201,122,0.07)",border:"1px solid rgba(0,201,122,0.2)",borderRadius:8,fontSize:"0.75rem",color:"#00c97a",fontWeight:600,textAlign:"center"}}>
              ✓ No significant postural deviations detected in {view} view
            </div>
          )}
        </div>
      )}
...
// ─── PosturePhotoCapture — replaces old version ───────────────────────────────
function PosturePhotoCapture({ videoRef, canvasRef, videoSize, isReady, measurements, analysis, currentView, sessionHistory, landmarks, isActive }) {
  const captureCanvasRef = useRef(null);
  const [captures, setCaptures] = useState([]);
  const [captureMode, setCaptureMode] = useState("auto");

  const applyAutoEnhance = (ctx, w, h) => {
    const img=ctx.getImageData(0,0,w,h); const d=img.data; let sum=0;
    for(let i=0;i<d.length;i+=4) sum+=0.299*d[i]+0.587*d[i+1]+0.114*d[i+2];
    const lum=sum/(d.length/4);
    const adj=lum<80?(80-lum)*0.55:lum>185?-(lum-185)*0.4:0;
    for(let i=0;i<d.length;i+=4){ d[i]=Math.min(255,Math.max(0,d[i]+adj)); d[i+1]=Math.min(255,Math.max(0,d[i+1]+adj)); d[i+2]=Math.min(255,Math.max(0,d[i+2]+adj)); }
    ctx.putImageData(img,0,0);
  };

  const doCapture = useCallback((mode="manual") => {
    const video=videoRef.current, overlay=canvasRef.current;
    if(!video||!videoSize) return;
    const {w,h}=videoSize;
    if(!captureCanvasRef.current) captureCanvasRef.current=document.createElement("canvas");
    const c=captureCanvasRef.current; c.width=w; c.height=h;
    const ctx=c.getContext("2d");
    ...
    const label=`${currentView?.toUpperCase()} · ${time}`;
    ctx.fillStyle="rgba(0,0,0,0.55)"; ctx.fillRect(5,h-26,ctx.measureText(label).width+10,20);
    ctx.fillStyle="#00e5ff"; ctx.fillText(label,10,h-11);
    const img=c.toDataURL("image/jpeg",0.92);

    // Run AI analysis immediately
    const postureAnalysis = runPostureCaptureAnalysis(measurements, currentView, landmarks);
    const postureScore = postureAnalysis?.postureScore ?? 100;

    const captureData={ img, time, view:currentView, captureMode:mode, analysis:postureAnalysis, postureScore, measurements };
    setCaptures(prev=>[captureData,...prev]);
    sessionHistory.save(captureData);
  },[videoRef,canvasRef,videoSize,measurements,currentView,landmarks,sessionHistory]);
  ...
  return(
    <div>
      ...
      {captures.length>0&&(
        <div>
          <div style={{fontSize:"0.6rem",fontWeight:700,color:"#6b8399",textTransform:"uppercase",letterSpacing:"1px",marginBottom:8}}>
            📸 Captures ({captures.length}) — tap to view analysis
          </div>
          {captures.map((cap,i)=>(
            <PostureCaptureCard
              key={i}
              capture={cap}
              index={i}
              onRetake={()=>doCapture("manual")}
              onDelete={()=>setCaptures(prev=>prev.filter((_,idx)=>idx!==i))}
            />
          ))}
        </div>
      )}
...
// ─── 30+ Posture Deviations List ─────────────────────────────────────────────
const POSTURE_DEVIATIONS = [
  // Head & Cervical
  { id:"fhp",      region:"Head/Neck",    label:"Forward Head Posture",          icon:"🤕" },
  { id:"headtilt", region:"Head/Neck",    label:"Head Lateral Tilt",             icon:"↗" },
  { id:"headrot",  region:"Head/Neck",    label:"Head Rotation",                 icon:"🔄" },
  { id:"cerv_hyper",region:"Head/Neck",   label:"Cervical Hyperextension",       icon:"⬆" },
  // Shoulders
  { id:"sh_elevated",region:"Shoulders",  label:"Elevated Shoulder (L/R)",       icon:"⬆" },
  { id:"sh_protract",region:"Shoulders",  label:"Rounded/Protracted Shoulders",  icon:"🔵" },
  { id:"sh_retract", region:"Shoulders",  label:"Retracted Scapulae",            icon:"🔴" },
  { id:"wing_scap",  region:"Shoulders",  label:"Winged Scapula",                icon:"🪶" },
  // Thoracic
  { id:"kyphosis",   region:"Thoracic",   label:"Increased Kyphosis",            icon:"🌀" },
  { id:"flat_back",  region:"Thoracic",   label:"Flat Thoracic Spine",           icon:"─" },
  { id:"scoliosis_t",region:"Thoracic",   label:"Thoracic Scoliosis (L/R curve)",icon:"〰" },
  // Lumbar
  { id:"lordosis",   region:"Lumbar",     label:"Increased Lumbar Lordosis",     icon:"🌀" },
  { id:"flat_lumbar",region:"Lumbar",     label:"Reduced Lumbar Lordosis",       icon:"─" },
  { id:"scoliosis_l",region:"Lumbar",     label:"Lumbar Scoliosis",              icon:"〰" },
  // Pelvis
  { id:"ant_tilt",   region:"Pelvis",     label:"Anterior Pelvic Tilt",          icon:"⬇" },
  { id:"post_tilt",  region:"Pelvis",     label:"Posterior Pelvic Tilt",         icon:"⬆" },
  { id:"lat_tilt",   region:"Pelvis",     label:"Lateral Pelvic Tilt (Trendelenburg)",icon:"↗" },
  { id:"pelvic_rot", region:"Pelvis",     label:"Pelvic Rotation",               icon:"🔄" },
  // Hips
  { id:"hip_flex",   region:"Hips",       label:"Hip Flexor Tightness",          icon:"⬆" },
  { id:"hip_ir",     region:"Hips",       label:"Hip Internal Rotation",         icon:"↩" },
  { id:"hip_er",     region:"Hips",       label:"Hip External Rotation",         icon:"↪" },
  // Knees
  { id:"k_valgus",   region:"Knees",      label:"Knee Valgus (Knock Knees)",     icon:"X" },
  { id:"k_varus",    region:"Knees",      label:"Knee Varus (Bow Legs)",         icon:"O" },
  { id:"k_hyperext", region:"Knees",      label:"Knee Hyperextension",           icon:"⬇" },
  { id:"k_flex",     region:"Knees",      label:"Knee Flexion Contracture",      icon:"⬆" },
  // Ankles & Feet
  { id:"pronation",  region:"Feet",       label:"Overpronation / Flat Feet",     icon:"⬇" },
  { id:"supination", region:"Feet",       label:"Supination / High Arch",        icon:"⬆" },
  { id:"plantar",    region:"Feet",       label:"Plantar Fascia Tightness",      icon:"🦶" },
  { id:"ankle_eq",   region:"Feet",       label:"Ankle Equinus",                 icon:"⬇" },
  // Trunk / Core
  { id:"trunk_lean_l",region:"Trunk",     label:"Lateral Trunk Lean (L/R)",      icon:"↗" },
  { id:"trunk_rot",  region:"Trunk",      label:"Trunk Rotation",                icon:"🔄" },
  { id:"upper_cross",region:"Patterns",   label:"Upper Crossed Syndrome",        icon:"✕" },
  { id:"lower_cross",region:"Patterns",   label:"Lower Crossed Syndrome",        icon:"✕" },
  { id:"layered",    region:"Patterns",   label:"Layered Syndrome",              icon:"📚" },
];

// ─── DeviationSelector — manual 30+ posture deviations checklist ─────────────
function DeviationSelector({ selected, onChange }) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const regions = [...new Set(POSTURE_DEVIATIONS.map(d=>d.region))];
  const filtered = search ? POSTURE_DEVIATIONS.filter(d=>d.label.toLowerCase().includes(search.toLowerCase())||d.region.toLowerCase().includes(search.toLowerCase())) : POSTURE_DEVIATIONS;
  ...
  return(
    <div style={{background:"#0d1117",border:"1px solid #1a2d45",borderRadius:12,padding:"10px 13px",marginBottom:10}}>
      <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",background:"transparent",border:"none",cursor:"pointer",padding:0}}>
        <span style={{fontSize:"0.62rem",fontWeight:700,color:"#7f5af0",textTransform:"uppercase",letterSpacing:"1px"}}>📋 Posture Deviations ({selected.length} selected)</span>
      ...
      {selected.length>0&&(
        <div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:7}}>
          {selected.map(id=>{
            const d=POSTURE_DEVIATIONS.find(x=>x.id===id);
            if(!d) return null;
            return(
              <span key={id} onClick={()=>onChange(selected.filter(s=>s!==id))} style={{fontSize:"0.64rem",padding:"2px 8px",borderRadius:7,background:"rgba(127,90,240,0.15)",color:"#7f5af0",border:"1px solid rgba(127,90,240,0.35)",cursor:"pointer",fontWeight:600}}>
                {d.icon} {d.label} ✕
              </span>
            );
          })}
      ...
      {open&&(
        <div style={{marginTop:10}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search deviations…"
            style={{width:"100%",background:"#131c28",border:"1px solid #1a2d45",borderRadius:8,color:"#d4e0f0",padding:"7px 11px",fontSize:"0.76rem",fontFamily:"inherit",outline:"none",marginBottom:9}}/>
          {regions.map(region=>{
            const items=filtered.filter(d=>d.region===region);
            if(items.length===0) return null;
            return(
              <div key={region} style={{marginBottom:9}}>
                <div style={{fontSize:"0.58rem",fontWeight:700,color:"#6b8399",textTransform:"uppercase",letterSpacing:"1px",marginBottom:5}}>{region}</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                  {items.map(d=>{
                    const sel=selected.includes(d.id);
                    return(
                      <button key={d.id} onClick={()=>onChange(sel?selected.filter(s=>s!==d.id):[...selected,d.id])} style={{padding:"5px 9px",background:sel?"rgba(127,90,240,0.18)":"transparent",border:`1px solid ${sel?"rgba(127,90,240,0.45)":"#1a2d45"}`,borderRadius:8,color:sel?"#7f5af0":"#6b8399",fontSize:"0.68rem",fontWeight:sel?700:500,cursor:"pointer",transition:"all 0.15s"}}>
                        {d.icon} {d.label}
                      </button>
                    );
                  })}
...
// ─── PostureAnalysisEngine (v3 — full rebuild with deviation selector) ────────
function PostureAnalysisEngine({ landmarks, canvasRef, videoSize, videoRef, isActive }) {
  ...
  return(
    <div style={{marginTop:14}}>
      ...
      {/* 30+ Deviation selector */}
      <DeviationSelector selected={selectedDeviations} onChange={setSelectedDeviations}/>

      {/* Selected deviations summary */}
      {selectedDeviations.length>0&&(
        <div style={{background:"rgba(127,90,240,0.06)",border:"1px solid rgba(127,90,240,0.2)",borderRadius:10,padding:"10px 13px",marginBottom:10}}>
          <div style={{fontSize:"0.6rem",fontWeight:700,color:"#7f5af0",textTransform:"uppercase",letterSpacing:"1px",marginBottom:8}}>🩺 Clinician-Marked Deviations</div>
          {selectedDeviations.map(id=>{
            const d=POSTURE_DEVIATIONS.find(x=>x.id===id);
            if(!d) return null;
            return(
              <div key={id} style={{display:"flex",gap:8,padding:"5px 0",borderBottom:"1px solid rgba(127,90,240,0.1)",fontSize:"0.73rem",color:"#d4e0f0"}}>
                <span style={{color:"#7f5af0"}}>{d.icon}</span>
                <span style={{fontWeight:600}}>{d.label}</span>
                <span style={{marginLeft:"auto",fontSize:"0.6rem",color:"#6b8399"}}>{d.region}</span>
              </div>
            );
          })}
        </div>
      ...
      <PosturePhotoCapture
        videoRef={videoRef} canvasRef={canvasRef} videoSize={videoSize}
        isReady={true} measurements={measurements} analysis={current}
        currentView={view} sessionHistory={sessionHistory}
        landmarks={landmarks} isActive={isActive}
      />

      {/* Summary modal */}
      {showSummary&&<AssessmentSummaryPanel captures={{}} measurements={measurements} symmetry={analysis?.symmetry} onClose={()=>setShowSummary(false)}/>}
      ...
      <div style={{marginTop:8,fontSize:"0.6rem",color:"#6b8399",padding:"6px 10px",background:"#06090f",borderRadius:7,border:"1px solid #1a2d45",lineHeight:1.5}}>
        ⚠ Observational guidance only. Not a clinical diagnosis. Findings are based on 2D camera perspective.
      </div>
    </div>
  );
}
