...
// ─── CaptureHoldTimer — configurable countdown with stability check ───────────
function CaptureHoldTimer({ onComplete, isReady, holdSecs = 3 }) {
  const [counting, setCounting] = useState(false);
  const [count, setCount] = useState(0);
  useEffect(()=>{
    if(!counting) return;
    if(count>=holdSecs){ setCounting(false); setCount(0); onComplete(); return; }
    const t=setTimeout(()=>setCount(c=>c+1),1000);
    return()=>clearTimeout(t);
  },[counting,count,onComplete,holdSecs]);
  const start=()=>{ if(!isReady||counting) return; setCounting(true); setCount(0); };
  const cancel=()=>{ setCounting(false); setCount(0); };
  const pct=counting?(count/holdSecs)*100:0;
  return(
    <div>
      {counting?(
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
            ...
            <span style={{fontSize:"0.78rem",fontWeight:800,color:"#00e5ff",minWidth:20}}>{holdSecs-count}s</span>
          </div>
          <div style={{display:"flex",gap:7}}>
            <div style={{flex:1,padding:"10px",background:"rgba(0,229,255,0.1)",border:"1px solid rgba(0,229,255,0.3)",borderRadius:10,textAlign:"center",fontSize:"0.75rem",fontWeight:800,color:"#00e5ff"}}>⏳ Hold still… {holdSecs-count}s</div>
            <button onClick={cancel} style={{padding:"10px 14px",background:"#1a2d45",border:"1px solid #1a2d45",borderRadius:10,color:"#6b8399",fontWeight:700,fontSize:"0.72rem",cursor:"pointer"}}>✕</button>
          </div>
        </div>
      ):(
        <button onClick={start} disabled={!isReady} style={{width:"100%",padding:"11px",background:isReady?"linear-gradient(135deg,#00e5ff,#7f5af0)":"#1a2d45",border:"none",borderRadius:10,color:isReady?"#000":"#6b8399",fontWeight:800,fontSize:"0.78rem",cursor:isReady?"pointer":"not-allowed"}}>
          {isReady?`📸 Capture (${holdSecs}s hold)`:"⏳ Waiting for stable tracking…"}
        </button>
      )}
    </div>
  );
...
// ─── PosturePhotoCapture — HD capture with auto-enhance, burst, export ────────
function PosturePhotoCapture({ videoRef, canvasRef, videoSize, isReady, measurements, analysis, currentView, sessionHistory }) {
  ...
  // Auto-enhance: brightness/contrast correction via canvas filter
  const applyAutoEnhance = (ctx, w, h) => {
    ...
    const brightnessAdj = lum < 80 ? (80 - lum) * 0.6 : lum > 180 ? -(lum - 180) * 0.4 : 0;
    for (let i = 0; i < d.length; i += 4) {
      d[i]   = Math.min(255, Math.max(0, d[i]   + brightnessAdj));
      d[i+1] = Math.min(255, Math.max(0, d[i+1] + brightnessAdj));
      d[i+2] = Math.min(255, Math.max(0, d[i+2] + brightnessAdj));
    }
    ctx.putImageData(img, 0, 0);
  };

  const doCapture = useCallback((forBurst=false) => {
    const video=videoRef.current, overlayCanvas=canvasRef.current;
    if(!video||!videoSize) return null;
    const {w,h}=videoSize;
    ...
    ctx.fillRect(6, h-28, ctx.measureText(`${currentView?.toUpperCase()} · ${time}`).width+12, 22);
    ctx.fillStyle="#00e5ff";
    ctx.fillText(`${currentView?.toUpperCase()} · ${time}`, 12, h-12);
    ...
    if(!forBurst){
      ...
      if(compMode){ if(!baseline) setBaseline(captureData); else setFollowUp(captureData); }
    }
    return img;
  ...
  const startBurst = useCallback(() => {
    ...
    setIsBursting(true); setBurstImages([]);
    let count=0;
    const t=setInterval(()=>{
      const img=doCapture(true);
      if(img) setBurstImages(b=>[...b,img]);
      count++;
      if(count>=5){ clearInterval(t); setIsBursting(false); }
    },400);
  },[doCapture,isBursting]);
  ...
  return(
    <div style={{background:"#0d1117",border:"1px solid #1a2d45",borderRadius:12,padding:"11px 13px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9,flexWrap:"wrap",gap:6}}>
        <div style={{fontSize:"0.6rem",fontWeight:700,color:"#6b8399",textTransform:"uppercase",letterSpacing:"1px"}}>📸 HD Capture · Auto-Enhance</div>
        <div style={{display:"flex",gap:6}}>
          <button onClick={()=>setCompMode(m=>!m)} style={{padding:"3px 9px",background:compMode?"rgba(0,229,255,0.15)":"transparent",border:`1px solid ${compMode?"rgba(0,229,255,0.4)":"#1a2d45"}`,borderRadius:7,color:compMode?"#00e5ff":"#6b8399",fontSize:"0.62rem",fontWeight:700,cursor:"pointer"}}>⇄ Compare</button>
          <button onClick={startBurst} disabled={!isReady||isBursting} style={{padding:"3px 9px",background:isBursting?"rgba(255,179,0,0.15)":"transparent",border:`1px solid ${isBursting?"rgba(255,179,0,0.4)":"#1a2d45"}`,borderRadius:7,color:isBursting?"#ffb300":"#6b8399",fontSize:"0.62rem",fontWeight:700,cursor:"pointer"}}>💥 Burst×5</button>
        </div>
      </div>

      {compMode&&(<ComparisonViewer baseline={baseline} followUp={followUp} onClear={()=>{setBaseline(null);setFollowUp(null);}}/>)}

      {/* Burst strip */}
      {burstImages.length>0&&(
        <div style={{marginBottom:9}}>
          <div style={{fontSize:"0.58rem",color:"#ffb300",fontWeight:700,marginBottom:4}}>💥 Burst Captures ({burstImages.length})</div>
          <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:4}}>
            {burstImages.map((img,i)=>(
              <div key={i} style={{position:"relative",flexShrink:0}}>
                <img src={img} alt={`burst-${i}`} style={{height:70,borderRadius:7,display:"block",border:"1px solid #1a2d45"}}/>
                <a href={img} download={`burst-${i+1}-${currentView}-${Date.now()}.jpg`} style={{position:"absolute",bottom:2,right:2,background:"rgba(0,201,122,0.85)",borderRadius:4,padding:"1px 5px",fontSize:"0.55rem",color:"#000",fontWeight:800,textDecoration:"none"}}>⬇</a>
              </div>
            ))}
          </div>
        </div>
      ...
      {!captured?(
        <CaptureHoldTimer onComplete={()=>doCapture(false)} isReady={isReady}/>
      ):(
        <div>
